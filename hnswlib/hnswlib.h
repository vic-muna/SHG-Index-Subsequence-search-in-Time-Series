...
class AlgorithmInterface {
 public:
    //virtual void addPoint(const void *datapoint, labeltype label, int flag, bool replace_deleted = false) = 0;
    ...
    virtual std::vector<std::pair<dist_t, labeltype>>
        searchKnnCloserFirst(const void* query_data, unsigned int, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const;
...
AlgorithmInterface<dist_t>::searchKnnCloserFirst(const void* query_data, unsigned int query_Id, size_t k,
                                                 BaseFilterFunctor* isIdAllowed) const {
    std::vector<std::pair<dist_t, labeltype>> result;

    // here searchKnn returns the result in the order of further first
    auto ret = searchKnn(query_data, query_Id, k, isIdAllowed);
    {
        size_t sz = ret.size();
        result.resize(sz);
        while (!ret.empty()) {
